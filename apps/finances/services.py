...
def record_rent_payment(*, lease_id: int, data: dict, recorded_by: User) -> RentPayment:
    ...
    with transaction.atomic():
        ...
        # Si le montant_attendu est déjà fourni (via le premier paiement auto ou la dette), on l'utilise
        # sinon on le calcule (Loyer + Charges + Dette)
        montant_attendu = data.get('montant_attendu')
        if not montant_attendu:
            montant_attendu = (lease.loyer_actuel or 0) + (lease.charges or 0) + dette
            
        montant_paye = data['montant_paye']
        reste = Decimal(str(montant_attendu)) - Decimal(str(montant_paye))
...
def record_expense(*, property_id: int, data: dict, created_by: User) -> Expense:
    ...
    payload = data.copy()
    # On s'assure que si categorie_id est présent, il est utilisé correctement
    cat_id = payload.pop('categorie_id', None)
